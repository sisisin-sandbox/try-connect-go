...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/sisisin-sandbox/try-connect-go/gen/greet/v1"
	http "net/http"
	strings "strings"
)
...
const (
	...
	// GreetServiceGreetErrorProcedure is the fully-qualified name of the GreetService's GreetError RPC.
	GreetServiceGreetErrorProcedure = "/greet.v1.GreetService/GreetError"
)
...
var (
	...
	greetServiceGreetErrorMethodDescriptor = greetServiceServiceDescriptor.Methods().ByName("GreetError")
)
...
type GreetServiceClient interface {
	...
	GreetError(context.Context, *connect.Request[v1.GreetRequest]) (*connect.Response[v1.GreetResponse], error)
}
...
func NewGreetServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) GreetServiceClient {
	...
	return &greetServiceClient{
		...
		greetError: connect.NewClient[v1.GreetRequest, v1.GreetResponse](
			httpClient,
			baseURL+GreetServiceGreetErrorProcedure,
			connect.WithSchema(greetServiceGreetErrorMethodDescriptor),
			connect.WithClientOptions(opts...),
		),
	}
}
...
type greetServiceClient struct {
	...
	greetError *connect.Client[v1.GreetRequest, v1.GreetResponse]
}
...
// GreetError calls greet.v1.GreetService.GreetError.
func (c *greetServiceClient) GreetError(ctx context.Context, req *connect.Request[v1.GreetRequest]) (*connect.Response[v1.GreetResponse], error) {
	return c.greetError.CallUnary(ctx, req)
}
...
func NewGreetServiceHandler(svc GreetServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	...
	greetServiceGreetErrorHandler := connect.NewUnaryHandler(
		GreetServiceGreetErrorProcedure,
		svc.GreetError,
		connect.WithSchema(greetServiceGreetErrorMethodDescriptor),
		connect.WithHandlerOptions(opts...),
	)
	return "/greet.v1.GreetService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case GreetServiceGreetErrorProcedure:
			greetServiceGreetErrorHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
...
func (UnimplementedGreetServiceHandler) GreetError(context.Context, *connect.Request[v1.GreetRequest]) (*connect.Response[v1.GreetResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("greet.v1.GreetService.GreetError is not implemented"))
}
